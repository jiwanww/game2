...
html='<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>능력전선 0.7.2 베타 · 인터넷 멀티와 요원 해금</title><style>'+css+'</style><body><canvas id="game"></canvas><div id="ui"></div><script type="module">'+loader+'</script></body></html>'
(root/'AbilityFront.html').write_text(html);(out/'AbilityFront.html').write_text(html)
version=json.loads((root/'package.json').read_text())['version']
archive=out/('AbilityFront-Beta-'+version+'.zip')
with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as z:
    for folder,dirs,names in os.walk(root):
        ...
        for name in sorted(names):
            ...
            if name=='local.properties' or name.startswith(('OWNER-CODE','private-owner')) or p.suffix in ['.jks','.keystore','.p12','.pem','.mobileprovision'] or name.startswith('.') and name!='.gitignore':continue
            z.write(p,'AbilityFront-LAN/'+str(rel))
print('Built',len(sources),'modules;',archive.name,archive.stat().st_size,'bytes')
